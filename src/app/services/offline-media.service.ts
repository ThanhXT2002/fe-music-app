...
export class OfflineMediaService {
  ...
  async debugTestThumbnailStorage(): Promise<void> {
    ...
    try {
      ...
      ], { type: 'image/png' });      console.log('💾 Saving test thumbnail...');
      let saveSuccess = false;

      // Use unified IndexedDB for all platforms
      saveSuccess = await this.databaseService.saveThumbnailBlob(testSongId, testBlob);
