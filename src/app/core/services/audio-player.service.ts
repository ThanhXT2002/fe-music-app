...
import { Song, PlaybackState } from '@core/interfaces/song.interface';
import { SavedPlaybackState } from '@core/interfaces/playback-state.interface';
...
/**
 * Tương tác trực tiếp với HTML5 Audio Web API và hệ điều hành.
 *
 * Nhiệm vụ:
 * - Khởi tạo môi trường phát (`HTMLAudioElement`).
 * - Tương tác Native API (CapacitorMusicControls, Media Session API).
 * - Xử lý stream (đọc Audio Blob từ IndexedDB ra URL tạm).
 * - Theo dõi bộ đệm (buffering) và báo cáo lỗi mạng hoặc giải mã.
 *
 * @note Tầng Component KHÔNG gọi trực tiếp service này, mà thông qua `PlayerStore`.
 */
/**
 * Chịu trách nhiệm tương tác trực tiếp với HTML5 Audio Web API.
 *
 * - Liên kết vòng lặp trạng thái với `PlayerStore`.
 * - Hỗ trợ Native Media Session API (điều khiển qua màn hình khóa / tai nghe).
 * - Xử lý luồng stream (blob audio) từ bộ nhớ thiết bị.
 */
export class AudioPlayerService {
  ...
  private handlePlaybackError(error: any, song: Song): void {
    ...
    console.warn(`Cannot play ${song.title}:`, error?.message || error);
    ...
    try {
      if (this.audio.paused && this._playbackState().currentSong) {
        ...
        if (!this.audio.src || this.audio.src === '') {
          console.warn('No audio source, reloading current song...');
