...
import { Song, DataSong, SongsResponse } from '@core/interfaces/song.interface';
import {
  HttpErrorResponse,
  HttpEvent,
  HttpEventType,
...
import { MusicApiService } from '@core/api/music-api.service';
...
/**
 * Tr�nh qu?n l� h? th?ng t?i nh?c (Download Manager).
 *
 * Nhi?m v?:
 * - Theo d�i ti?n tr�nh t?i b�i h�t th�ng qua HTTP Events.
 * - X? l� Fallback linh ho?t: N?u API Stream proxy th?t b?i, t? d?ng chuy?n v? ch? d? Background Polling.
 * - Sau khi t?i xong, t? d?ng d?y d? li?u d?ng Blob v�o `IndexedDBService`.
 */
@Injectable({
  providedIn: 'root',
})
/**
 * Quản lý hệ thống tải nhạc (Download Manager).
 *
 * - Theo dõi tiến trình tải các bài hát (progress tracking).
 * - Xử lý fallback chuyển đổi giữa Proxy Stream và Backend Polling.
 * - Xử lý lưu trữ File Blob trực tiếp vào CSDL IndexedDB.
 */
export class DownloadService {
  ...
  private async initializeDownloads() {
    try {
      ...
      if (isInitialized) {
        ...
        console.warn(
          'IndexedDB initialization failed, downloads will not persist'
        );
      }
    } catch (error) {
  ...
  private async downloadAudio(
    ...
    try {
      ...
      console.warn(`Proxy download thất bại cho ${songId}, thử endpoint cũ:`, proxyError);
  ...
  private async saveSongToDatabase(songData: DataSong) {
    try {
      ...
      if (success) {
        // Note: UI refresh is handled by DownloadStore signal reactivity
      } else {
  ...
  private async saveDownloadsToIndexedDB() {
    try {
      ...
      if (!isInitialized) {
        console.warn(
          'IndexedDB initialization failed, skipping download saving'
        );
        return;
      }
  ...
  private async loadDownloadsFromIndexedDB() {
    try {
      ...
      if (!isInitialized) {
        console.warn(
          'IndexedDB initialization failed, skipping download loading'
        );
        return;
      }
  ...
  //         console.warn(
  //           `Status check failed (${attempt}/${maxAttempts}):`,
  ...
  //           return true;
  //         } else {
  //           console.warn('Download task not found for song:', songId);
  //           return false;
  //         }
  ...
  //   console.warn('Max polling attempts reached, song may not be ready');
  //   return false;
  // }
    ...
    const pollStatus = async () => {
      try {
        ...
        if (statusResponse.success) {
          ...
          console.warn('Status check failed:', statusResponse.message);
        }
      } catch (error) {
